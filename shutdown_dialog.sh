...
if [ -n "${ACTION}" ];then
	case $ACTION in
		Shutdown)
			#zenity --question --text "Are you sure you want to shutdown?" && /usr/lib/indicator-session/gtk-logout-helper -s
			...
			;;
		Reboot)
			#zenity --question --text "Are you sure you want to reboot?" && /usr/lib/indicator-session/gtk-logout-helper -r
